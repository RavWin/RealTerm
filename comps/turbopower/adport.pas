 ...
 *  Sebastian Zierer
*  sjb Add support for using Windows Createfile names as well as port numbers
* sjb    pass ecADWSCANTRESOLVE error through along with file errors, probably to deal with an unplugging error
* sjb 17Jun17 Added OnError to deal with EInOutErr caused by unplugging usb ports when open. AdPort,AwUser,AdExcept,AdWnPort
*    based on example by KATTES https://sourceforge.net/p/tpapro/discussion/241883/thread/2f274722/?limit=25#7f19/74a0
 *
 * ***** END LICENSE BLOCK ***** *)
...
type
  ...
  protected {private}
    ...
    FComName         : string;              //sjb add way to use windows filename
    ...
    FOnErrorEvent         : TErrorHandlerEvent;   {Error occured}          // CG - V5.01
    ...
    procedure SetComName(NewName : string);  //sjb
    ...
    procedure SetOnErrorEvent(const Value: TErrorHandlerEvent);                                      // SWB
    ...
    function MakeComName(const ComNum: Word): String;
  protected
    {Misc}
    function CheckException(const Ctl : TComponent;
                               const Res : Integer) : Integer; overload;            //sjb  CG - V5.01
    function CheckException(const Res: Integer): Integer; overload; //incase  we want to no use all the "selfs"
  ...
  public
    ...
    property ComName : string //sjb Added so ports can be opened by filename not just number.
      read FComName write SetComName;
    ...
    property OnErrorEvent :  TErrorHandlerEvent                            // CG - V5.01
      read FOnErrorEvent write SetOnErrorEvent;
...
var
  ...
  procedure TApdCustomComPort.SetComName(NewName: string); //sjb Added to support named comports
  var
    WasOpen : Boolean;
    OldTracing : TTraceLogState;
    OldLogging : TTraceLogState;
  begin
    if (NewName[1]<>'\') and (NewName[2]<>'\')  //name can be either full path or just the port Name i.e \\.\COM4 or COM4
      then NewName:='\\.\'+NewName;

    if FComName <> NewName then begin
  ...
  begin
    if FComNumber <> NewNumber then begin
      ...
      FComNumber := NewNumber;
      FComName:=MakeComName(FComNumber);
      if WasOpen then begin
        Tracing := OldTracing;
        Logging := OldLogging;
        Open := True;
  ...
  procedure TApdCustomComPort.SetOnErrorEvent(
    const Value: TErrorHandlerEvent);                                      // CG - V5.01
  begin
    FOnErrorEvent := Value;
    if Assigned(FDispatcher) then   //sjb  not assigned until opened.
       FDispatcher.SetErrorHandler(Value);
  end;
...
function TApdCustomComPort.MakeComName(const ComNum : Word) : String;
      {-Return a string like 'COMXX'}
      var Temp : array[0..12] of Char;
    begin
      if TapiMode <> tmOn then begin
        StrFmt(Temp, '\\.\COM%d', [ComNum]);
        Result := Temp;
      end else
        Result := ''; //sjb was nil . Will this matter?
    end;
...
//    end;
  ...
  begin
    ...
    Result := Dispatcher.InitPort(pchar(FComName), FBaud,
  ...
  begin
    ...
    FDispatcher := ActivateDeviceLayer;
        FDispatcher.DeviceName := Format('COM%d', [ComNumber]);  //sjb FDispatcher.DeviceName := Format('\\.\COM%d', [ComNumber]);             // SWB
    if Assigned(FOnErrorEvent) then FDispatcher.SetErrorHandler(FOnErrorEvent);  //sjb dispatcher didn't exist when port was created
    try
      ...
      if Res = ecAccessDenied then
        Res := ecAlreadyOpen
      else if (Res = ecFileNotFound) or (Res = ecPathNotFound)
        or (res=-ecADWSCANTRESOLVE) //sjb was not passing error through to be caught
       then
        Res := ecBadId;
  ...
  begin
    if (PortState = psShuttingDown) or (Dispatcher=nil) then Exit;
...
{Waits}
  function TApdCustomComPort.CheckException(const Ctl: TComponent;
             const Res: Integer): Integer;                                // CG - V5.01
  begin
    result := AdExcept.CheckException(Ctl, Res, FOnErrorEvent); //sjb
  end;
  function TApdCustomComPort.CheckException(const Res: Integer): Integer;                                 // CG - V5.01
  begin
    result := AdExcept.CheckException(Self, Res, FOnErrorEvent); //sjb
  end;
