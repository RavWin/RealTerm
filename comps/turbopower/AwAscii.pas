 ...
 *
 * sjb 29may15  
 *     Made ASCII (60) and Binary (1023) BLOCKLEN different to speed up file sending
 *     Fixes to correctly send blocks
 *
 * ***** END LICENSE BLOCK ***** *)
...
function spSetBlockLen(P : PProtocolData; NewLen : Cardinal) : Integer;
...
const
  {Compile time constants} //seem to fight with awp ones in AwProtcl...
  ...
  awaDefBlockLen = 1023; //sjb        {Note: must be less than SizeOf(TDataBlock)-1}
...
{Ascii protocol}
  ...
  begin
    with P^ do
      if aCurProtocol <> Ascii then
        spSetBlockLen := ecBadProtocolFunction
      else begin
        if (NewLen <= apMaxBlockSize-1) then begin
          spSetBlockLen := ecOK;
          aBlockLen := NewLen;
        end else begin
          spSetBlockLen := ecPacketTooLong;
          aBlockLen := apMaxBlockSize-1;
        end;
      end;
  end;
            ...
            case sAsciiState of
              ...
              taGetBlock :
                begin
                  ...
                  if aNoMoreData then aLastBlockSize:=aBytesRemaining; //sjb fix up end blocks
