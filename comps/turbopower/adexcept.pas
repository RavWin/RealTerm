 ...
 *    Sean B. Durkin
 *    Sebastian Zierer
 * sjb 17Jun17 Added Port.OnError to deal with EInOutErr caused by unplugging usb ports when open. AdPort,AwUser,AdExcept,AdWnPort
...
type
  ...
  TErrorHandlerEvent = procedure (CP : TObject; ErrCode : Integer) of object;    //sjb matches wserrorhandler TErrorHandlerEvent = procedure (CP : TObject; ErrCode : Integer) of object;    //sjb matches wserrorhandler
  ...
  function CheckException(const Ctl : TComponent;
                          const Res : Integer;
                          const ErrorHandler : TErrorHandlerEvent = nil) : Integer; //sjb added error handler capability here for AdPort and Dispatcher
...
uses
  ...
  //function CheckException(const Ctl : TComponent; const Res) : Integer;
  function CheckException(const Ctl : TComponent;
                          const Res : Integer;
                          const ErrorHandler : TErrorHandlerEvent = nil) : Integer; //sjb added error handler capability here for AdPort and Dispatcher
  ...
  begin
    ...
    if (Res < ecOk) then
      if not (csLoading in Ctl.ComponentState) then begin
        if assigned(ErrorHandler) then begin  //sjb use components FOnErrorEvent if assigned
          ErrorHandler(Ctl, Res);  //Should TErrorHandlerEvent return RES so it can clear/alter it???
          exit;
        end;
