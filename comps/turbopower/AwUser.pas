 ...
 * SJB 27Sep2017        - trap errors when putblock called on closed port, when
 *                        errorhandler is used. This cannot be avoided by checking
 *                        port.open when autoopen is being used
 *
 *  Sulaiman Mah
 *  Sean B. Durkin
 *  Sebastian Zierer
...
type
  ...
  TErrorHandlerEvent = procedure (CP : TObject; E : Integer) of object;  // CG - V5.01
  ...
  TApdBaseDispatcher = class
    ...
    public
      ...
      procedure SetErrorHandler(const Value: TErrorHandlerEvent);          // CG - V5.01
...
{$ENDIF}
  ...
  begin
    try                                                                     // SWB
       AdExcept.CheckException(TComponent(Owner),                           // SWB
                       TApdDispatcherThread(Sender).ReturnValue,            // SWB
                        FOnErrorEvent);                                     //sjb                                                                    // SWB
    except                                                                  // SWB
...
{Routines used by constructor}
  ...
  begin
    if (self=nil) then begin   //sjb if port does not auto-open, can be called and will memory error
      Result := ecCommNotOpen; //don't throw an error here as ValidDispatcher has already done so
      exit;
    end;
...
//      AbortDispatchLogging;                                               // SWB
  ...
  procedure TApdBaseDispatcher.SetErrorHandler(const Value: TErrorHandlerEvent); // CG - V5.01
  begin
    FOnErrorEvent := Value;
  end;
