 ...
 * Contributor(s):
 *  Sebastian Zierer
 * sjb Add property CopyAddCRLF. Originally, clipboard copy from terminal always adds CRLF, even when copying a small chunk in the middle of a line
 * sjb Fix terminal Enter key behaviour so CR,LF,CRLF can be sent. 
 *     Add EnterSendCRLF property. 
 *     Change Keymap to proper VT100 with backspace key, null etc. (old keymap sends #127 for BS)
 *     Stop Terminal blocking menu keys.
 * sjb Fix nullchar being lost (happens when uisng AllChars emulator and  font set with char at 0
 *     Don't reset curson when changing teminal lines.
 *     Add CommandList to protected of AdTerminalEmulator so descendents can access it.
 *     11Mar17 Allow terminal to autoopen port
 *     30Jun17 Add published property HasLineAtEdgeOfCols : Draws a thin vertical gray line at the edge of the terminal columns
 * sjb 29Jul17 For Telnet added teSendString, CRLF must be sent together (in same putblock) due to CR replaced by CR NUL in Telnet
 *     see http://www.freesoft.org/CIE/RFC/1123/31.htm
 *     see https://sourceforge.net/p/realterm/bugs/83/
 * ***** END LICENSE BLOCK ***** *)
...
type                          
  ...
  TAdCustomTerminal = class(TApdBaseWinControl)
    private
      ...
      FCopyAddsCRLF          : boolean;   //sjb 13May15
      FHasLineAtEdgeOfCols   : boolean;   //sjb 30Jun17
   ...
   {published}                                                         
      ...
      property CopyAddsCRLF :boolean                                     //sjb
                read FCopyAddsCRLF write FCopyAddsCRLF  default false;
      ...
      property HasLineAtEdgeOfCols : boolean       //sjb Draws a this vertical gray line at the edge of the terminal columns
                  read FHasLineAtEdgeOfCols write FHasLineAtEdgeOfCols
                  default false;
  ...
  TAdTerminalEmulator = class(TApdBaseComponent)
    private
      ...
      FReturnKeySendsCRLF :boolean;
    procedure teSendString(aStr: AnsiString; aCanEcho: boolean;
      CharSource: TAdCharSource);                  //sjb
    ...
    protected
      ...
      property CommandList :TAdEmuCommandList read FCommandList write FCommandList; //added sjb for descendents
    public
      constructor Create(aOwner : TComponent); override;
      destructor Destroy; override;
    ...
    published
      ...
      property ReturnKeySendsCRLF: boolean read FReturnKeySendsCRLF write FReturnKeySendsCRLF default false;
  ...
  TAdTerminal = class(TAdCustomTerminal)
    published
      ...
      property Columns;
      property ComPort;
      property CopyAddsCRLF;                                             //sjb
      property CursorType;
      property FollowCursor;                                             {!!.03}
      property HalfDuplex;
      property HasLineAtEdgeOfCols;   //sjb
...
begin
  ...
  aInx := aInx + FTotalLineCount; //trying to find crash when changing scrollback rows.
  aInx := aInx - FEmulator.Buffer.RowCount - 1;
...
begin
  ...
  //sjb more correct, but sizeof(lineattr) is 1 anyway, so no actual effect
  if (NewTotalCount < FTotalLineCount) then  //shrinking
    Move(FArray[(FTotalLineCount - NewTotalCount)* sizeof(TAdVT100LineAttr)] , NewArray[0],
         NewTotalCount * sizeof(TAdVT100LineAttr))
  else {FTotalLineCount <= NewTotalCount}   //expanding
    Move(FArray[0], NewArray[(FTotalLineCount - NewTotalCount)* sizeof(TAdVT100LineAttr)],
         FTotalLineCount * sizeof(TAdVT100LineAttr));
  {free the old array, save the new one}
//sjb trying to fix crash on change scrollback - no luck
  FreeMem(FArray, FTotalLineCount * sizeof(TAdVT100LineAttr));
  FArray := NewArray;
...
{--------}
function TAdCustomTerminal.IsMenuKey(var Message: TWMKey): Boolean;
begin
  Result := True;
  if not (csDesigning in ComponentState) then
  begin
      if Assigned(PopupMenu) and (PopupMenu.WindowHandle <> 0) and
       PopupMenu.IsShortCut(Message)
        then Exit;
  end;
  Result := False;
end;
...
begin
  {if the left button is down (we're selecting) and the key is Escape,
   cancel the mode}
  UpdateUIState(Msg.CharCode);
  if IsMenuKey(Msg) then begin
      Msg.Result := 1;
      Exit;
  end;
...
begin
  ...
  if FCopyAddsCRLF then begin //originally always adds it - even when selecting a small piece in middle of line
  TextPtr[CurInx] := ^M;
  TextPtr[CurInx+1] := ^J;
  inc(CurInx, 2);
  end;
...
begin                                                                    {!!.03}
  ...
  if Assigned (FEmulator) then begin                                     {!!.03}
   ...
   //   FEmulator.Buffer.Row := 1;        //sjb resetting cursor when window size changes                                 {!!.03}
    end;                                                                 {!!.03}
  end;                                                                   {!!.03}
  inherited Loaded;                                                      {!!.05}
end;                                                                     {!!.03}
...
var
  ...
  PaintRect : TRect;
  PaintRgn  : HRGN;
    procedure DrawEdgeLine; //sjb Draws a this vertical gray line at the edge of the terminal columns
        var X:integer;
    begin
        X:=FCharWidth * Columns + 4; //FCharWidth div 2; //+8; //right edge beyond last char
        with Canvas do begin
            Pen.Color:= clDkGray;
            MoveTo(X, 0);
            LineTo(X, PaintRect.Bottom );
        end;
    end;
...
begin
  ...
  try
    ...
    if FHasLineAtEdgeOfCols then               //sjb
      DrawEdgeLine;
...
procedure TAdTerminalEmulator.KeyDown(var Key : word; Shift: TShiftState);
  procedure SendAndClearKey(C:AnsiChar);
  begin
    teSendChar(C, true, csKeyboard);
    Key := 0;
  end;
begin
//sjb fix up Enter key behaviour
  if (Key=13) then begin //enter key
    if (Shift=[ssCtrl])
      then teSendChar(#10, true, csKeyboard);
    if (Shift=[ssShift])
      then begin
        teSendString(#13+#10, true, csKeyboard);  //sjb for telnet
//        teSendChar(#13, true, csKeyboard);
//        teSendChar(#10, true, csKeyboard);
    end;
    if (Shift=[])
      then begin
//        teSendChar(#13, true, csKeyboard);
//        if ReturnKeySendsCRLF then teSendChar(#10, true, csKeyboard);
        if ReturnKeySendsCRLF
          then teSendString(#13+#10, true, csKeyboard)  //sjb for telnet
          else teSendChar(#13, true, csKeyboard);
      end;
    Key := 0;  //Doesn't work if using WM_KEYDOWN
  end;
  //sjb change/fix backspace & delete (verriding KeyPressed function below)
  if (Key=08) and (Shift=[])  //backspace key
    or (Key=72) and (Shift=[ssCtrl])   //^H
    then SendAndClearKey(#08);
  if (Key=08) and ((Shift=[ssShift]) or (Shift=[ssCtrl]))  //shift backspace
    or (Key=46) and (Shift=[])   //delete
    then SendAndClearKey(#$7F); //DEL
  //sjb add Null Key
  if (Key=50) and (Shift=[ssCtrl])  //^@
    or (Key=32) and (Shift=[ssCtrl])   //^space
    then SendAndClearKey(#0);
end;
{--------}
...
begin
  ...
  if (Terminal <> nil) then begin
    ...
    if (Terminal.ComPort <> nil) and (Terminal.ComPort.Open or Terminal.Comport.AutoOpen) then //sjb 11Mar17 Allows terminal to autoopen port
      Terminal.ComPort.PutChar(aCh);
  end;
end;
procedure TAdTerminalEmulator.teSendString (aStr        : AnsiString;           {!!.04}
                                          aCanEcho   : boolean;        {!!.04}
                                          CharSource : TAdCharSource); {!!.04}
begin
  {Assumptions: aCh is a printable character or is one of the standard
  ...
  if (Terminal <> nil) then begin
    if aCanEcho and Terminal.HalfDuplex then
      ProcessBlock(@aStr, length(aStr), CharSource);                               {!!.04}
    //if (Terminal.ComPort <> nil) and Terminal.ComPort.Open then
    if (Terminal.ComPort <> nil) and (Terminal.ComPort.Open or Terminal.Comport.AutoOpen) then //sjb 11Mar17 Allows terminal to autoopen port
      Terminal.ComPort.PutString(aStr);
  end;
end;
{--------}
...
begin
  ...
  while (Walker <> nil) do begin
    ...
    ExtTextOutA(Canvas.Handle,    //sjb 5Mar17 changed from ExtTextOut so NUL char0 works
               WorkRect.Left,
               WorkRect.Top,
               ETO_OPAQUE,
               @WorkRect,
               FDisplayStr, //sjb warning with string(FDisplayStr)
               TextStrLen,
               @FCellWidths^);
...
begin
  ...
  FKeyboardMapping.LoadFromRes(hInstance, 'ADVT100_BS_KeyMap');  //sjb this is an actal vt100 keymap with BS key sends BS
