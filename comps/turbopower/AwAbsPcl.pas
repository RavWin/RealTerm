 ...
 *
 * Contributor(s):
 * 20May15  add aSendFileName    hack to send send file going in aapPrepareReading
...
const
  ...
  begin
    with P^ do begin
      aSendFileName:=FName; //sjb hack to get filename working in
...
      Assign(aWorkFile, string(aSendFileName));      //sjb Assign(aWorkFile, string(aPathName));
      Reset(aWorkFile, 1);
      FileMode := aSaveMode;
      Res := IOResult;
      if Res <> 0 then begin
