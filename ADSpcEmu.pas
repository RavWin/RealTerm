...
implementation
//procedure TAdShowAllEmulator.KeyDown(var Key : word; Shift: TShiftState); //override;
//var
//  Ch        : AnsiChar;
//begin
//  if (Key=13) then begin //enter key
//    if (Shift=[ssCtrl]) then teSendChar(#10, true, csKeyboard);
//    if (Shift=[ssShift]) then begin
//        teSendChar(#13, true, csKeyboard);
//        teSendChar(#10, true, csKeyboard);
//    end;
//    if (Shift=[]) then teSendChar(#13, true, csKeyboard);
...
var
  ...
  Str        : ansistring;                                                   {!!.03}
  StrLen     : Integer;                                                  {!!.03}
  ThisCharIsCR, ThisCharIsLF, ThisCharIsLF2 : boolean;
...
begin
  ...
  for i := 0 to pred (aDataLen) do begin
    ThisCharIsCR:=false;
    ThisCharIsLF:=false;
    ThisCharIsLF2:=LastCharWasLF and ThisCharIsLF;
    Ch := DataAsChar[i];
    SetLength (Str, 1);                                                  {!!.03}
    Str[1] := Ch;                                                        {!!.03}
          ...
          case Ch of                                                          {!!.04}
            ...
            ^J : begin//LF
                 if ssShowCRLF in FControlCharShowStyles
                   then begin
                     if LastCharWasCR //and not LFSinceLastRealChar
                ...
                LFSinceLastRealChar:=not LFSinceLastRealChar;
                end;
            ^M : begin//CR
                 if ssShowCRLF in FControlCharShowStyles
                   then begin
                     if LastCharWasLF or LastCharWasCR //or not LFSinceLastRealChar  //in the special case of CR CR at end of line....
                      then PutAtEndOfLastLine(Ch)
                      else SavePosWriteChar(Ch);                                               {!!.04}
                     //storep
                   end;
...
//              if LastCharWasCR and (buffer.col=1) then begin
//                PutAtEndOfLastLine(^M);
//                //Buffer.Col:=1;
//              end;                                                               {!!.04}
              WriteRealChar(Ch);
              //LastCharWasCR:=false;
...
//    end; //if
    //LastCharWasCR:=LastCharWasCR or ThisCharIsCR;
    //LastCharWasLF:=(LastCharWasLF or ThisCharIsLF) and not ThisCharIsLF2;                                                                {!!.03}
