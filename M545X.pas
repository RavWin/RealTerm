...
function Str2M5451D4(S:string):ansistring; //Scrambled I2CCHIP segment arrangement

function Str2NSM4000(S:string):ansistring; //Unscrambled Natsemi obsolete display


function ClearM5451D4Str:ansistring; //Command string to clear M5451 . Must be sent after power on, before first data str

implementation
uses StBits,{math,} sysutils;
...
function Str27Seg(S: string) : ansistring ;
...
begin
  ...
  result:=ansichar(0)+ansichar(0)+ansichar(0)+ansichar(0)+ansichar(0);
  R5:=0;
  for i:=length(S) downto 1 do begin
    if digitnum<=4
      then begin
          HoldDigitNum:=false;
          Ch:=ansichar(S[i]); //get the char
          case Ch of
          ...
          result[5-digitnum]:= ansichar(byte(result[5-digitnum]) or R); //or dp in
          if not HoldDigitNum
      ...
      //      HoldDigitNum:=true; //stall at digit 5    [DCC Hint] M545X.pas(171): H2077 Value assigned to 'HoldDigitNum' never used
            Ch:=ansichar(S[i]); //get the char
            case Ch of
              ...
              else ;//do nothing R5:=0; //unknown chars are blank
            end; //case
          end; //digitnum=6

    //R:=reverseBitOrder(R);
  end; //for
   result[5]:=ansichar(R5);
end; //Str27Seg
...
function String2Hex(S:ansistring) :ansistring;
...
function Str2M5451D4(S:string):ansistring;
{returns the hex chars to send to an M5451D4 display via BL233}
begin
  result:=Str27Seg(S);
...
function Str2NSM4000(S:string):ansistring;
{returns the hex chars to send to an M5451D4 display via BL233}
begin
  result:=Str27Seg(S);
  //don't map
...
function Str2M5451D4viaBL233(S:string) :ansistring ;
begin
