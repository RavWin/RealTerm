...
interface
  ...
  function IAscii2Hex(S:string; CompactAscii:boolean=false):string;
  function IBL233EEWriteStr(Address:word; DataStr:string; IsText:boolean=true; CompactAscii:boolean=false):string;
  ...
  //function WriteFile2I2CMem(Type,Filename)
  //function ReadI2CMEM(Type,Filename, StartAddress, NumBytes
...
//  {$include I2CEEProm_Definitions.pas }
//   const
//     I2CMemTable : array[0..1] of TI2CMem = ((Name: '24xx00'; SizeBytes: 64; PointerBytes: 1; 1,5) , ('24xx01,128,1,1,5) );
(*  type tHexData = class
    private
...
//write internal eeprom of BL233
function IBL233EEWriteStr(Address:word; DataStr:string; IsText:boolean=true; CompactAscii:boolean=false):string;
  //var S:string;
begin
  if IsText then DataStr:=IAscii2Hex(DataStr,CompactAscii);
  result:='V'+inttohex(Address,2)+inttohex( $FF-Address,2)+DataStr+'.';
end;
...
//begin
//  //showmessage('UNIMPLEMENTED function: HexN2Int);' +
//
//end;
...
//    function ProcessHexSubString(S:string; FormatStr:string='u'):string;
//      procedure TrimLeadingCharX(var S:string; IsBigEndian, IsSigned:boolean; IsFloat:boolean=false);
//      var STrimmed:string; i:integer;
//      begin
//        BigEndian:=IsBigEndian;
...
var IsAllHex:boolean;
    ...
    SSFormat:string;
    iFormat:integer; //
    procedure  SSEndProcess;   //SubStringEndProcess
    begin
...
//              inc(iSubstring);
              IsAllHex:=true;
              SS:='';
              result:=result+c; //put delimiter on end of output string....
            end
            else begin  //not delimiter
              if C>=' ' then begin
              SS:=SS+c;
              case C of
                '0'..'9','A'..'F' : ;
